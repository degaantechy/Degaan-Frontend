...
export function calculateConstructionEstimate(values, rates = CONSTRUCTION_RATES) {
  ...
  const standardBoundaryLength = Math.max(0, perimeter - gateOpeningWidth)
  const decorativeBoundaryArea = decorativeBoundaryLength * boundaryHeight
  const standardBoundaryArea = standardBoundaryLength * boundaryHeight
  const boundaryCost = (
    standardBoundaryArea * rates.boundaryStandard
  ) + (
