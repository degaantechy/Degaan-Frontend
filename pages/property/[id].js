...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        trackAnalyticsEvent('property_detail_view', { property_id: id })
      } catch (error) {
  ...
  const canonical = `https://www.degaanrealestate.com/property/${id}`
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:title" content={`${address} | Degaan Real Estate`} />
        ...
        <div className="property-detail">
          <div className="property-images">
            {property.images && property.images.length > 0 ? (
              <div className="main-image"><img src={property.images[0]} alt={address} /></div>
            ) : (
          ...
          <div className="property-info">
            ...
            <div className="property-specs">
              <div className="spec"><strong>{t('property.size')}:</strong> {property.size} {t('property.areaUnit')}</div>
              <div className="spec"><strong>{t('property.bedrooms')}:</strong> {property.bedrooms}</div>
              <div className="spec"><strong>{t('property.location')}:</strong> {location}</div>
              <div className="spec"><strong>{t('property.type')}:</strong> {getPropertyType(property.property_type, language)}</div>
            </div>

            <div className="property-description"><h3>{t('property.description')}</h3><p>{description}</p></div>

            <div className="property-features">
              <h3>{t('property.features')}</h3>
              <ul>{features && features.split(',').map((feature, idx) => <li key={idx}>{feature.trim()}</li>)}</ul>
            </div>
            ...
            <div className="property-actions">
              <a
                ...
                onClick={() => trackAnalyticsEvent('whatsapp_click', { placement: 'property_detail', property_id: id })}
              >
                {t('property.contactWhatsapp')}
              </a>
              <Link href={`/contact?property=${id}`} className="btn-secondary" onClick={() => trackAnalyticsEvent('property_enquiry_start', { property_id: id })}>
